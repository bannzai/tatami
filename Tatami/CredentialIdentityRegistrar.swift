...
/// 識別子はサイトのホスト名・ユーザー名・レコード ID (Credential.id) だけで、パスワードは登録しない (充填時に拡張が Keychain から読む)
        ...
        let identities = credentials
            ...
            .map { credential in
                ASPasswordCredentialIdentity(
                    serviceIdentifier: ASCredentialServiceIdentifier(identifier: credential.host, type: .domain),
